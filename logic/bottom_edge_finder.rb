...
class BottomEdgeFinder < FinderBase
  def initialize(radar, noise_level_threshold, max_roll_down)
  ...
  def find(invader)
    ...
    (1..max_roll_down).each do |current_offset|
      start_radar_row = radar.data.height - invader.data.height + current_offset
      (0..max_column).each do |column_idx|
        noise_level = invader.data.with_each_coord.sum do |invader_vector|
          next 0 if invader_vector.y > invader.data.height - current_offset - 1
          radar_signal = radar.data.at(column_idx + invader_vector.x, invader_vector.y + start_radar_row)
